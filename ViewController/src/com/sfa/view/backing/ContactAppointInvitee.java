...
public class ContactAppointInvitee {
    public ContactAppointInvitee() {
        super();
    }
    ...
    public List attributeListForIteratorContacts() {
        ...
        DCIteratorBinding iter = bc.findIteratorBinding("ContactAppInviteeVO1Iterator");
        if(iter == null) {
...
//        }
    ...
    public BigDecimal getCurrentInteractionId() {
        DCBindingContainer bc = (DCBindingContainer) getBindings();
        //        AttributeBinding attr = (AttributeBinding)bc.findCtrlBinding("AppointmentId");

        DCIteratorBinding iterBind = (DCIteratorBinding) bc.get("ContactAppointmentsVO2Iterator");
    ...
    public void onCommit(ActionEvent actionEvent) {
        ...
        DCBindingContainer bc = (DCBindingContainer) getBindings();
        DCIteratorBinding iter1 = bc.findIteratorBinding("ContactAppointmentsVO2Iterator");
        AttributeBinding accountId = (AttributeBinding) bc.getControlBinding("CustAccountId");
        Row currentRow = iter1.getCurrentRow();
        currentRow.setAttribute("CustAccountId", accountId);
        ...
        if (size > 0) {
            for (int i = 0; i < size; i++) {
                ...
                row.setAttribute("AppointmentId", getCurrentInteractionId());
